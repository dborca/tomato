...
void start_ipsec(void)
{
	...
	if (with_cert) {
		char *p = nvram_safe_get("ipsec_ca");
		if (*p == '/') {
			symlink(p, "/tmp/ipsec/ca.crt");
		} else {
		...
		p = nvram_safe_get("ipsec_crt");
		if (*p == '/') {
			symlink(p, "/tmp/ipsec/server.crt");
		} else {
		...
		p = nvram_safe_get("ipsec_key");
		if (*p == '/') {
			symlink(p, "/tmp/ipsec/server.key");
		} else {
